...
class MovenetModule(yarp.RFModule):
    ...
    def configure(self, rf):
        ...
        while not yarp.Network.checkNetwork():
            print("Waiting for yarpserver to start.")
            sleep(2)
        #
    ...
    def check_external_port(self, port):
        result = yarp.Network.exists(port)
        if not result:
    ...
    def test_and_connect(self):
        count = 0
        while not yarp.Network.checkNetwork():
            print("Waiting for yarpserver to start.",count)
            count += 1
            sleep(2)
        print('Yarp server is running. Checking ports....')
        input_ext='/edpr_vojext/eros:o'
        output_ext='/edpr_vojext/movenet:i'
        while not self.check_external_port(input_ext):
            sleep(1)
        ...
        if not (yarp.Network.isConnected('/edpr_vojext/eros:o', '/movenet/img:i')):
            yarp.Network.connect('/edpr_vojext/eros:o', '/movenet/img:i', 'tcp')

        if not(yarp.Network.isConnected('/movenet/sklt:o', '/edpr_vojext/movenet:i')):
            yarp.Network.connect('/movenet/sklt:o', '/edpr_vojext/movenet:i', 'tcp')

        # print('port',yarp.Network.exists('/vojext'))
        # yarp.Network.isConnected()
        # <<inputput>>.Port.getOutputCount()
        # print(yarp.Network.connect('/vojext','/vojext','fast_tcp'))
    ...
    def updateModule(self):
        ...
        if not (yarp.Network.isConnected('/edpr_vojext/eros:o', '/movenet/img:i', True)):
